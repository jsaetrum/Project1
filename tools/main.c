...
LRESULT CALLBACK MEChildWndProc(HWND hwnd, UINT Message, WPARAM wParam,LPARAM lParam);

char g_szAppName[] = "MapEditorWindow";
char g_szChild[] = "MapEditorChild";
HINSTANCE g_hInst;
...
BOOL GetFileName(HWND hwnd, LPSTR pszFileName, BOOL bSave) {
	...
	ofn.hwndOwner = hwnd;
	ofn.lpstrFilter = "Map Files (*.map)\0*.map\0All Files (*.*)\0*.*\0\0";
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,LPSTR lpszCmdParam, int nCmdShow) {
	...
	if(!RegisterClassEx(&WndClassEx)) {
		MessageBox(0, "Could Not Register Window", "Error.",MB_ICONEXCLAMATION | MB_OK);
		return -1;
	}

	WndClassEx.lpfnWndProc     = MEChildWndProc;
   ...
   WS_EX_APPWINDOW,g_szAppName,"Map File Editor v 1.0",
	...
	switch(Message) {
		case WM_CREATE: {
         ...
         "mdiclient", NULL,
			...
			SendMessage(g_hStatusBar, SB_SETTEXT, 2, (LPARAM)"MapTool");
		...
		case WM_COMMAND: {
			switch(LOWORD(wParam)) {
				...
				case CM_FILE_NEW: {
					...
					if(!hChild) {
						MessageBox(hwnd, "Map Editor Child creation failed.", "Error.",MB_ICONEXCLAMATION | MB_OK);
					}
					break;
				}
				case CM_FILE_OPEN: {
					...
					if(!hChild) {
						MessageBox(hwnd, "Map Editor Child creation failed.", "Error.",
						MB_ICONEXCLAMATION | MB_OK);
					}
					break;
				}
...
//        {
//            PAINTSTRUCT ps;
//            HDC hdc = BeginPaint (g_hMDIClient, &ps);
//            TextOut (hdc,
